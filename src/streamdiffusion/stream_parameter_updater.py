...
class StreamParameterUpdater(OrchestratorUser):
    ...
    def _recalculate_timestep_dependent_params(self, t_index_list: List[int]) -> None:
        ...
        if self.stream.kvo_cache and old_batch_size != self.stream.batch_size:
            ...
            # Resize replaces per-layer tensors with fresh standalone allocations,
            # detaching them from the bucketed storage. Drop the bucket refs so
            # update_kvo_cache falls back to per-layer writes against the new tensors.

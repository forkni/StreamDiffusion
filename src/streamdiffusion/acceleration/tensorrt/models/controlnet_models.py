...
from ..export_wrappers.unet_sdxl_export import SDXLConditioningHandler
...
class ControlNetTRT(BaseModel):
    ...
    def get_sample_input(self, batch_size, image_height, image_width):
        ...
        return (
            ...
            torch.randn(batch_size, 77, self.embedding_dim, dtype=dtype, device=self.device),
            torch.randn(
...
def create_controlnet_model(
    model_type: str = "sd15", unet=None, model_path: str = "", conditioning_channels: int = 3, **kwargs
) -> ControlNetTRT:
    """Factory function to create appropriate ControlNet TensorRT model"""
    if model_type.lower() in ["sdxl"]:
        return ControlNetSDXLTRT(
            unet=unet, model_path=model_path, conditioning_channels=conditioning_channels, **kwargs
        )
    else:

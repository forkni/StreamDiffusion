...
class UNet2DConditionModelEngine:
    def __init__(self, filepath: str, stream: "cuda.Stream", use_cuda_graph: bool = False):
        ...
        # Pre-allocated template dicts for __call__ hot path — updated in-place each call
        # to avoid per-call dict creation and f-string formatting overhead.
        # KVO key names are populated lazily on first call (model-dependent length).
    ...
    def __call__(
        ...
        **kwargs,
        ...
        shape_dict["sample"] = latent_model_input.shape
        shape_dict["timestep"] = timestep.shape
        shape_dict["encoder_hidden_states"] = encoder_hidden_states.shape
        shape_dict["latent"] = latent_model_input.shape

        input_dict["sample"] = latent_model_input
        input_dict["timestep"] = timestep
        input_dict["encoder_hidden_states"] = encoder_hidden_states

        for i, _kvo in enumerate(kvo_cache):
            in_name = self._kvo_in_names[i]
            out_name = self._kvo_out_names[i]
        ...
        if controlnet_conditioning is not None:
            ...
            self._add_controlnet_residuals(
                down_block_additional_residuals, mid_block_additional_residual, shape_dict, input_dict
            )
        else:
            ...
            if self.use_control:
                ...
                if unet_arch:
                    ...
                    if (
                        self._cached_dummy_controlnet_inputs is None
                        or not hasattr(self, "_cached_latent_dims")
                        or self._cached_latent_dims != (current_latent_height, current_latent_width)
                    ):
                        try:
                            self._cached_dummy_controlnet_inputs = self._generate_dummy_controlnet_specs(
                                latent_model_input
                            )
                    ...
                    if self._cached_dummy_controlnet_inputs is not None:
                        self._add_cached_dummy_inputs(
                            self._cached_dummy_controlnet_inputs, latent_model_input, shape_dict, input_dict
                        )

...
class DepthAnythingTensorrtPreprocessor(BasePreprocessor):
    ...
    def _process_core(self, image: Image.Image) -> Image.Image:
        ...
        # Auto-normalize TRT output to [0, 1], then apply category-standard depth grade.
        depth_min, depth_max = float(depth.min()), float(depth.max())
        depth_norm = (depth - depth_min) / max(depth_max - depth_min, 1e-6)
        depth_norm = apply_depth_grade_numpy(
            depth_norm,
            gamma=float(self.params.get("gamma", 1.0)),
            black_level=float(self.params.get("black_level", 0.0)),
            white_level=float(self.params.get("white_level", 1.0)),
            invert=bool(self.params.get("invert", False)),
        )
        depth = (depth_norm * 255.0).astype(np.uint8)
    ...
    def _process_tensor_core(self, image_tensor: torch.Tensor) -> torch.Tensor:
        ...
        depth_normalized = apply_depth_grade(
            depth_normalized,
            gamma=float(self.params.get("gamma", 1.0)),
            black_level=float(self.params.get("black_level", 0.0)),
            white_level=float(self.params.get("white_level", 1.0)),
